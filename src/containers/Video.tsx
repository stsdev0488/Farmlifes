...
export default ({ props }) => {
  ...
  let feedData = feeds.feeds.filter(c => (c && c.attachment && c.attachment.type) === 'video')
  ...
  const [waitLoadMore, onChangeWaitLoadMore] = useState(true);
  ...
  useEffect(() => {
    try {
      dispatch(getFeeds({ page: 1, type: 'video', userId: userInfo.user.user.id }));
    }
    catch (err) {
  ...
  handleReload = () => {
    ...
    dispatch(getFeeds({ page: page + 1, type: 'video', reload: true, userId: userInfo.user.user.id }));
  ...
  handleLoadMore = async () => {
    if (!feeds.interactionLoadingState.loadMore) {
      if (!(feeds.feedType === 'video' && !!feeds.endLoadMore)) {
        onChangeWaitLoadMore(true)
        if (page === 1 && feedData.length < 5) {
          onChangePage(page + 1)
          return await dispatch(getFeeds({ page: page + 1, type: 'video', userId: userInfo.user.user.id }));
        }
        else {
          onChangePage(page + 1)
          await dispatch(getFeeds({ page: page + 1, type: 'video', userId: userInfo.user.user.id }));
        }
      }
    }
  };
  ...
  return (
    <Feed
      ...
      headerBack={true}
      headerName={strings('Drawer.video')}
      props={props}
      shift={shift}
      displayAds={true}
    />
