...
export default ({ props }) => {
  ...
  let feedData = feeds.feeds.filter(c => c.type === 'question')
  ...
  useEffect(() => {
    if (feedData.length < 1) {
      try {
        dispatch(getFeeds({ page: 1, type: 'question', userId: userInfo.user.user.id }));
      }
      catch (err) {
  ...
  handleReload = () => {
    ...
    dispatch(getFeeds({ page: 1, type: 'question', reload: true, userId: userInfo.user.user.id }));
  ...
  handleLoadMore = async () => {
    if (!feeds.interactionLoadingState.loadMore) {
      if (!(feeds.feedType === 'question' && !!feeds.endLoadMore)) {
        if (page === 1 && feedData.length < 5) {
          onChangePage(page + 1)
          return dispatch(getFeeds({ page: page + 1, type: 'question', userId: userInfo.user.user.id }));
        }
        onChangePage(page + 1)
        return dispatch(getFeeds({ page: page + 1, type: 'question', userId: userInfo.user.user.id }));
      }
  ...
  return (
    <Feed
      ...
      props={props}
      headerBack={true}
      headerName={strings('Drawer.forum')}
      shift={shift}
      displayAds={true}
    />
  );
